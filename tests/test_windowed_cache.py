...
def _drive_cache(scores_per_call, B=1, H_kv=2, D=8):
    """Drive a WindowedCache through prefill + 2 decode steps, injecting
    per-call window scores. Geometry (window_size=1, num_sink=0, local=1,
    budget=0.375, prefill=8) → top_k=2 evictable + 1 local; the eviction fires
    on the 2nd decode call. Keys encode their token index, so the returned
    sorted survivor list *is* the set of retained window indices."""
    cfg = WindowedCacheConfig(
...
class TestScoreAccumulation:
    """Cumulative window scores accumulate across prefill and decode; scores
    earned while a window is local persist after it ages into the evictable
    band."""
    ...
    def test_local_window_score_accumulates_into_evictable(self):
        ...
        surv = _drive_cache([
            _score({i: 0.1 for i in range(8)} | {7: 0.2}, 8),
            _score({8: 100.0}, 9),   # scored while local
            _score({}, 10),          # eviction fires; token 8 now evictable
        ])
        assert surv == [7, 8, 9]
