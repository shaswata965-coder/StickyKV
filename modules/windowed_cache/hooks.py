    ...
    for _name, module in model.named_modules():
        ...
        def make_hook(lidx: int):
            def score_hook(module, args, kwargs, output):
                ...
                #    H2O-style cumulative scoring: sum the softmax attention
                #    over the query rows on every pass (prefill AND decode). The
                #    cache accumulates these per-window sums continuously across
                #    chunks, steps, and windows. Softmax is computed in fp32 so
                #    tiny probabilities do not underflow before the sum.
                ...
                # 4. Reduce to per-window cumulative scores and hand off to the
                #    cache, which accumulates them across steps.
