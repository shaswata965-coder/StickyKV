...
def compute_window_scores(
    ...
    """Reduce full attention weights to per-window cumulative scores.

    Algorithm:
    1. Reduce over T query rows → per-key received attention ``Σ_i A_ij``
       giving ``[B, H_q, S]`` (plain H2O cumulative sum). The cache accumulates
       these across prefill and decode (they are additive).
    ...
    -------
    Tensor
        Shape ``[B, H_q, W]`` — per-window cumulative received attention.
        Sink tokens are **not** represented.
    """
    # 1. Reduce over T query rows → per-key received attention [B, H_q, S]
    #    (plain H2O cumulative sum). The cache accumulates these continuously
    #    across prefill and decode (they are additive).
